...
export class WxWorkGroupService extends WxWorkAuthServiceBase {
  ...
  async create (name: string, owner: string, userList: string[], chatId = '') {
    return await this.bin.post('/cgi-bin/appchat/create', { name, owner, userlist: userList, chatid: chatId }, { access_token: await this.getToken() }) as WxWork.CreateGroupResponse
  }
  ...
  async update (chatId: string, name: string, owner: string, addUserList: string[], delUserList: string[]) {
    ...
    return await this.bin.post('/cgi-bin/appchat/update', { chatid: chatId, name, owner, add_user_list: addUserList, del_user_list: delUserList }, param) as WxWork.NormalResponse
  }
