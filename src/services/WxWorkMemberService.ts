...
export class WxWorkMemberService extends WxWorkAuthServiceBase {
  ...
  async create (user: WxWork.User) {
    return await this.bin.post('/cgi-bin/user/create', $.snakeCaseKeys(_.cloneDeep(user)), { access_token: await this.getToken() }, {}, [60102, 60104]) as WxWork.NormalResponse
  }

  // 删除成员 https://work.weixin.qq.com/api/doc/90000/90135/90198
  async delete (useridlist: string[]) {
    return await this.bin.post('/cgi-bin/user/batchdelete', { useridlist }, { access_token: await this.getToken() }) as WxWork.NormalResponse
  }

  // 更新成员 https://work.weixin.qq.com/api/doc/90000/90135/90197
  async update (userId: string, user: WxWork.User) {
    return await this.bin.post('/cgi-bin/user/update', { userid: userId, ...user }, { access_token: await this.getToken() }) as WxWork.NormalResponse
  }
  ...
  async getJoinQr (size_type: number) {
    ...
    return await this.bin.get('/cgi-bin/corp/get_join_qrcode', { access_token: await this.getToken(), size_type })
  }

  // 获取部门成员详情 https://work.weixin.qq.com/api/doc/90000/90135/90201
  async getList (department_id = 1, fetch_child = 1) {
    const result = await this.bin.get('/cgi-bin/user/list', { access_token: await this.getToken(), department_id, fetch_child })
    return result as WxWork.UserList
  }

  // 通过ID获取用户 https://work.weixin.qq.com/api/doc/90000/90135/90196
  async getUserById (id: number) {
    const result = await this.bin.get('/cgi-bin/user/get', { access_token: await this.getToken(), id })
    return result as WxWork.User
  }

  // 获取访问用户身份 https://work.weixin.qq.com/api/doc/90000/90135/91023
  async getUserByCode (code: string) {
    const result = await this.bin.get('/cgi-bin/user/getuserinfo', { access_token: await this.getToken(), code })
  ...
  async getActiveUserStat (date: string) {
    return await this.bin.post('cgi-bin/user/get_active_stat', { date }, { access_token: await this.getToken() })
  }
}