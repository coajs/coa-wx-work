...
export class WxWorkMessageService extends WxWorkAuthServiceBase {
  ...
  async textCard ({ userIds = [] as [], deptIds = [] as number[], tagIds = [] as number[] } = {}, title: string, description: string, url: string) {
  ...
  async markdown ({ userIds = [] as string[], deptIds = [] as number[], tagIds = [] as number[] } = {}, markdown: WxWorkMarkdown) {
    const data = { msgtype: 'markdown', markdown: { content: markdown.value() } }
    return await this.send({ userIds, deptIds, tagIds }, data)
  }

  protected async send ({ userIds = [] as string[], deptIds = [] as number[], tagIds = [] as number[] } = {}, data: WxWork.Dic) {
    const touser = userIds.join('|')
    const toparty = deptIds.join('|')
    const totag = tagIds.join('|')
    return await this.bin.post('/cgi-bin/message/send', { agentid: this.agent.agentId, touser, toparty, totag, ...data }, { access_token: await this.getToken() })
  }
